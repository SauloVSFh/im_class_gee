...
// This script does Random FOREST based crop type classification based on Sentinel 2 and Sentinel 1
...
var numbers = ee.List.sequence (0.1, 1, 0.1);

var accuracies = numbers.map (function(t)
{
var classifier = ee.Classifier.smileRandomForest(300, 5 , 1, 0.6, null , 999)
.train({
  features: training,
...
var classifier = ee.Classifier.smileRandomForest(300,null, 1, 0.6, null, 999).train({
  features: training,
  classProperty: 'class_id',
  inputProperties: bands
});
...
Map.addLayer(mis, {}, 'mis');

// Export the Irrigated area Image to Geotiff

var x = classified.toArray()
print(classified)

var projection = season1.select('SR_B2').projection().getInfo();
// Export the Irrigated area Image to Geotiff
